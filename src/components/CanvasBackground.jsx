...
/**
 * Enhanced Canvas Backgrounds with Performance Optimizations
 * - Downsampled resolution based on performanceMode
 * - Throttled frame rates for complex animations
 * - Reduced particle counts in low-power mode
 */

// ─── Fireflies ───
function initFireflies(canvas, lowPower) {
  const ctx = canvas.getContext('2d');
  const particles = [];
  const COUNT = lowPower ? 15 : 40;
...
// ─── Rain ───
function initRain(canvas, lowPower) {
  const ctx = canvas.getContext('2d');
  const drops = [];
  const splashes = [];
  ...
  function draw() {
    ...
    for (const d of drops) {
      ...
      if (d.y > canvas.height) {
        if (!lowPower) splashes.push({ x: d.x, y: canvas.height - 2, life: 1, spread: Math.random() * 2 + 1 });
        d.y = -d.len;
...
void main() {
  ...
  vec3 col = mix(vec3(0.08, 0.02, 0.15), vec3(0.02, 0.08, 0.2), n1);
  col = mix(col, vec3(0.15, 0.04, 0.08), n2 * 0.6);
  col = mix(col, vec3(0.03, 0.12, 0.12), n3 * 0.4);
...
// ─── Waves ───
function initWaves(canvas, lowPower) {
  const ctx = canvas.getContext('2d');
  const layers = [
  ...
  if (!lowPower) layers.push({ y: 0.72, amp: 12, freq: 0.015, speed: 0.022, color: 'rgba(10, 40, 90, 0.3)' });
  ...
  function draw() {
    ...
    sky.addColorStop(0, '#050510'); sky.addColorStop(1, '#0a1525');
    ctx.fillStyle = sky; ctx.fillRect(0, 0, canvas.width, canvas.height);
    for (const l of layers) {
      ctx.beginPath(); const baseY = canvas.height * l.y; ctx.moveTo(0, canvas.height);
      for (let x = 0; x <= canvas.width; x += lowPower ? 10 : 3) {
        const y = baseY + Math.sin(x * l.freq + t * l.speed) * l.amp + Math.sin(x * l.freq * 1.8 + t * l.speed * 0.7) * l.amp * 0.4;
        ctx.lineTo(x, y);
      }
      ctx.lineTo(canvas.width, canvas.height); ctx.fillStyle = l.color; ctx.fill();
    }
    raf = requestAnimationFrame(draw);
  }
  raf = requestAnimationFrame(draw);
...
const INIT_MAP = { fireflies: initFireflies, rain: initRain, nebula: initNebula, waves: initWaves };
...
export default function CanvasBackground({ id }) {
  ...
  useEffect(() => {
    ...
    function resize() {
      // Resolution downsampling in performance mode (renders at 50% size)
      const scale = performanceMode ? 0.5 : 1;
    ...
    const cleanup = init(canvas, performanceMode);
  ...
  return (
    <canvas ref={canvasRef} className="background" style={{ background: '#050508' }} />
  );
}
